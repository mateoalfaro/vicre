...
Servicio de usuario que captura la pantalla, consulta al agente OpenCode 2 con las fuentes del curso y escribe la respuesta directamente donde esté escribiendo el usuario.
...
**Consulta**:
El ciclo completo: captura → prompt fijo + imagen adjunta enviados al agente OpenCode 2 → respuesta parseada y almacenada en memoria.
...
`~/.vicre/fuentes` para que el agente OpenCode 2 los consulte con grep/read.
_Avoid_: biblioteca, documentos, tarjetas

**Directorio de trabajo**:
`~/.vicre`: contiene `photos/`, `fuentes/` y es el directorio de trabajo desde el que corre el agente OpenCode 2.
