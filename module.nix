...
in
{
  options.programs.vicre = {
    ...
    model = lib.mkOption {
      ...
      description = "OpenCode provider/model that vicre uses for the consulta. Must support image input.";
    };
    variant = lib.mkOption {
      type = lib.types.str;
      default = "max";
      description = "Reasoning variant joined into --model as provider/model#variant for OpenCode 2 run.";
    };
    systemd = {
      enable = lib.mkOption {
        type = lib.types.bool;
